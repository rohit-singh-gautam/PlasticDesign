...
difference() {
    translate([-holder_width/2, -holder_thick/2, -holder_height/2])
        SmoothCube([holder_width, holder_thick, holder_height], holder_thick/2);
    ...
    union() {
        cube([pullout_width, material_thick, pullout_cube_height], center=true);
        translate([0, 0, pullout_cube_height/2])
            rotate([90, 0, 0])
            cylinder(h=material_thick, d=pullout_width, center=true);
        translate([0, 0, -pullout_cube_height/2])
            rotate([90, 0, 0])
            cylinder(h=material_thick, d=pullout_width, center=true);
    }
    ...
    union() {
        cube([notch_cube_width, material_thick, notch_height], center=true);
        translate([notch_cube_width/2, 0, 0])
            rotate([90, 0, 0])
            cylinder(h=material_thick, d=notch_height, center=true);
        translate([-notch_cube_width/2, 0, 0])
            rotate([90, 0, 0])
            cylinder(h=material_thick, d=notch_height, center=true);
    }
